...
* BREAKING CHANGE - The `cidrhost` function now requires a third argument if the IP should be returned in a CIDR notation.
...
* `cidrhost` now supports returning the calculated IP address in a CIDR notation.
